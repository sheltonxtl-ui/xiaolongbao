  ...
  const controlProps =
    typeof href === 'string'
      ...
      : typeof onClick === 'function'
        ? { onClick, disabled: Boolean(disabled) }
        : { disabled: true as const }

  return (
    <span className={clsx(className, 'grow basis-0')}>
      <Button {...controlProps} plain aria-label="Previous page">
...
export function PaginationNext({
  ...
  return (
    <span className={clsx(className, 'flex grow basis-0 justify-end')}>
      <Button {...controlProps} plain aria-label="Next page">
        {children}
        <svg className="stroke-current" data-slot="icon" viewBox="0 0 16 16" fill="none" aria-hidden="true">
          <path
...
export function PaginationPage({
  ...
  if (!linkMode && typeof onClick !== 'function') {
    throw new Error('PaginationPage requires either href or onClick')
  }
  ...
  return (
    <Button
      {...(linkMode
        ? { href }
        : { type: 'button' as const, onClick, disabled: current })}
      plain
