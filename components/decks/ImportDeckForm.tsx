...
export type ImportDeckFormProps = {
...
export function ImportDeckForm({
  ...
  const applyFile = useCallback((file: File | null) => {
    ...
    if (!isAllowedUploadExtension(file.name)) {
      setError("Unsupported file type. Choose a supported format and try again.");
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Could not import from that link. Try again or use a file instead.",
        );
      } finally {
  ...
  }, [linkUrl, method, onImport, selectedFile]);
    ...
    <Fieldset>
      ...
      >
        <RadioField>
          ...
          <Description>
            Paste a public URL to a Google Doc, a direct file link (PDF, Word, text), or a web
            page with study content.
          </Description>
        </RadioField>
        <RadioField>
          ...
          <Description>
            {formatAllowedUploadTypes()} — up to {formatFileSize(MAX_UPLOAD_BYTES)}.
          </Description>
        </RadioField>
      </RadioGroup>
      ...
      {method === "link" ? (
        <Field>
          ...
          <Description>
            Google Docs must be shared as “Anyone with the link can view.” Other pages are imported
            as plain text.
          </Description>
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3">
        {showCancel && cancelHref ? (
          <Button outline href={cancelHref}>
            {cancelLabel}
          </Button>
        ) : showCancel && onCancel ? (
