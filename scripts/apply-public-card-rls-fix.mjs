...
async function main() {
  ...
  const sqlPath = resolve(
    process.cwd(),
    "supabase/migrations/20260628140000_fix_public_card_rls.sql",
  );
  ...
  if (!databaseUrl) {
    console.error(
      [
        ...
        "",
        "Add one of these to .env.local, then rerun: npm run db:migrate:fix-public-cards",
        "",
        ...
        "",
        "Alternatively, paste supabase/migrations/20260628140000_fix_public_card_rls.sql",
  ...
  try {
    ...
    console.log("Applied public card RLS fix successfully.");
  } finally {
